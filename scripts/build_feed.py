...
def main() -> None:
    ...
    for r in rows:
        ...
        if image:
            item["image"] = image
        items.append(item)
    # Two posts pointing at one page are one item — the oldest keeps the
    # slot (that is when the link was first kept), and the later posts ride
    # along as more reasons it was.
    by_url: dict[str, dict] = {}
    for item in sorted(items, key=lambda i: i["date_published"]):
    ...
    items = sorted(by_url.values(), key=lambda i: (i["date_published"], i["id"]), reverse=True)
    feed = {
